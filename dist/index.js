(() => {
      ...
      var paper = function(self2, undefined) {
        ...
        var Base = new function() {
          ...
          return inject(Base2, {
            ...
            statics: {
              ...
              slice: function(list2, begin, end) {
                return slice.call(list2, begin, end);
              }
            }
          });
        }();
        ...
        Base.inject({
          ...
          statics: {
            ...
            read: function(list2, start, options, amount) {
              if (this === Base) {
                var value = this.peek(list2, start);
                list2.__index++;
                return value;
              }
              var proto = this.prototype, readIndex = proto._readIndex, begin = start || readIndex && list2.__index || 0, length = list2.length, obj = list2[begin];
              amount = amount || length - begin;
              if (obj instanceof this || options && options.readNull && obj == null && amount <= 1) {
                if (readIndex)
                  list2.__index = begin + 1;
                return obj && options && options.clone ? obj.clone() : obj;
              }
              obj = Base.create(proto);
              if (readIndex)
                obj.__read = true;
              obj = obj.initialize.apply(obj, begin > 0 || begin + amount < length ? Base.slice(list2, begin, begin + amount) : list2) || obj;
              if (readIndex) {
                list2.__index = begin + obj.__read;
                var filtered = obj.__filtered;
                if (filtered) {
                  list2.__filtered = filtered;
            ...
            peek: function(list2, start) {
              return list2[list2.__index = start || list2.__index || 0];
            },
            remain: function(list2) {
              return list2.length - (list2.__index || 0);
            },
            readList: function(list2, start, options, amount) {
              var res = [], entry, begin = start || 0, end = amount ? begin + amount : list2.length;
              for (var i = begin; i < end; i++) {
                res.push(Array.isArray(entry = list2[i]) ? this.read(entry, 0, options) : this.read(list2, i, options, 1));
              }
              return res;
            },
            readNamed: function(list2, name, start, options, amount) {
              var value = this.getNamed(list2, name), hasValue = value !== undefined;
              if (hasValue) {
                var filtered = list2.__filtered;
                if (!filtered) {
                  var source = this.getSource(list2);
                  filtered = list2.__filtered = Base.create(source);
                  filtered.__unfiltered = source;
                }
                filtered[name] = undefined;
              }
              return this.read(hasValue ? [value] : list2, start, options, amount);
            },
            readSupported: function(list2, dest) {
              var source = this.getSource(list2), that = this, read = false;
              if (source) {
                Object.keys(source).forEach(function(key) {
                  if (key in dest) {
                    var value = that.readNamed(list2, key);
                    if (value !== undefined) {
                      dest[key] = value;
                    }
                    read = true;
            ...
            getSource: function(list2) {
              var source = list2.__source;
              if (source === undefined) {
                var arg = list2.length === 1 && list2[0];
                source = list2.__source = arg && Base.isPlainObject(arg) ? arg : null;
              }
              return source;
            },
            getNamed: function(list2, name) {
              var source = this.getSource(list2);
              if (source) {
                return name ? source[name] : list2.__filtered || source;
              }
            },
            hasNamed: function(list2, name) {
              return !!this.getNamed(list2, name);
            },
            ...
            push: function(list2, items) {
              var itemsLength = items.length;
              if (itemsLength < 4096) {
                list2.push.apply(list2, items);
              } else {
                var startLength = list2.length;
                list2.length += itemsLength;
                for (var i = 0; i < itemsLength; i++) {
                  list2[startLength + i] = items[i];
                }
              }
              return list2;
            },
            splice: function(list2, items, index, remove) {
              var amount = items && items.length, append = index === undefined;
              index = append ? list2.length : index;
              if (index > list2.length)
                index = list2.length;
              for (var i = 0; i < amount; i++)
                items[i]._index = index + i;
              if (append) {
                Base.push(list2, items);
                return [];
              } else {
                var args = [index, remove];
                if (items)
                  Base.push(args, items);
                var removed = list2.splice.apply(list2, args);
                for (var i = 0, l = removed.length; i < l; i++)
                  removed[i]._index = undefined;
                for (var i = index + amount, l = list2.length; i < l; i++)
                  list2[i]._index = i;
                return removed;
              }
            },
              ...
              statics: {
                ...
                _clearBoundsCache: function(item) {
                  ...
                  if (cache) {
                    ...
                    for (var i = 0, list2 = cache.list, l = list2.length; i < l; i++) {
                      var other = list2[i];
                      if (other !== item) {
        ...
        var CompoundPath = PathItem.extend(
          {
            ...
            insertChildren: function insertChildren(index, items) {
              var list2 = items, first = list2[0];
              if (first && typeof first[0] === "number")
                list2 = [list2];
              for (var i = items.length - 1; i >= 0; i--) {
                var item = list2[i];
                if (list2 === items && !(item instanceof Path))
                  list2 = Base.slice(list2);
                if (Array.isArray(item)) {
                  list2[i] = new Path({ segments: item, insert: false });
                } else if (item instanceof CompoundPath) {
                  list2.splice.apply(list2, [i, 1].concat(item.removeChildren()));
                  item.remove();
                }
              }
              return insertChildren.base.call(this, index, list2);
            },
  ...
  var accordionAnimation = function() {
    ...
    const OPTION_FIRST_OPEN2 = "cr-accordion-first-open";
    ...
    if (accordionLists.length === 0)
      return;
    accordionLists.forEach((list2) => {
      let firstOpen = attr(false, list2.getAttribute(OPTION_FIRST_OPEN2));
      let oneActive = attr(false, list2.getAttribute(OPTION_ONE_ACTIVE));
      let keepOneOpen = attr(false, list2.getAttribute(OPTION_KEEP_ONE_OPEN));
      let hoverOnly = attr(false, list2.getAttribute(OPTION_HOVER));
      ...
      if (!hoverOnly) {
        list2.addEventListener("click", function(e) {
          ...
          const accordionItems = list2.querySelectorAll(ACCORDION_ITEM);
